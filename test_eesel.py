...
class TestAgentsCreateCommand:
    ...
    def test_add_is_hidden_alias_for_create(self, tmp_config, fake_creds, monkeypatch, capsys):
        # `agents add` behaves identically to `agents create`.
        calls = _capture_requests(monkeypatch)
        rc = eesel.cmd_agents(_parse("agents", "add", "--name", "QA Bot"))
        assert rc == 0
        assert calls[0]["body"]["name"] == "QA Bot"
        assert calls[0]["body"]["is_active"] is True

    def test_add_alias_parses_but_is_hidden(self):
        parser = eesel.build_parser()
        # `add` parses (runnable alias)...
        args = parser.parse_args(["agents", "add", "--name", "X"])
        assert args.agents_cmd == "add"
        # ...but is not advertised in the agents subcommand choices.
        agents_sub = _subparsers_action(parser).choices["agents"]
        visible = [a.dest for a in _subparsers_action(agents_sub)._choices_actions]
        assert "add" not in visible
        assert "create" in visible
    AGENTS = [
        {"agent_id": "agent-abc123", "name": "Support Bot"},
        {"agent_id": "agent-def456", "name": "Blog Writer"},
